...
class StaffSalary(BaseModel):
    ...
    def __str__(self):
        return {self.user.username} - {self.gross_salary}
    
class StaffSalaryPersontageCalculations(BaseModel):
    dearness_allowance_percentage = models.DecimalField(max_digits=10, decimal_places=2, default=0)
    house_rent_allowance_percentage = models.DecimalField(max_digits=10, decimal_places=2, default=0)
